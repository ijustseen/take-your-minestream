...
## Закреплённые сообщения (v3.0)

- ПКМ по сообщению закрепляет его в мире/на сервере.
- Удержание ПКМ перетаскивает закреплённое сообщение в любую точку перед игроком.
- ЛКМ удаляет только незакреплённые сообщения.
- Закреплённые сообщения не исчезают по таймеру и не участвуют в `follow player`.
